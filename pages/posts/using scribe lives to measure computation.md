...
A little while back I was having a conversation with a friend (1) and we came up with the idea of measuring computation in "scribe lives". This post introduces a new unit of measurement for computation, the "Scrife" (plural: "Scrives"). The general idea of this unit of measurement is that there should be some rough analogue between digital computation in FLOPs and scribe lives. Or in to put another way, for any given computational operation, we might ask: "How many ancient Babylonian scribes would I have to pay to work out this operation by hand, for their entire lives, in order to find the result?" It is approximately equal to ~186 million floating point operations. We can estimate that mining one bitcoin is approximately ~184 gigascrives.
{ .annotate }

1.  (I can't remember which of us thought of it, so credit also goes to [him](https://github.com/adamatbi) as well). This idea was inspired by [this blog post](https://www.righto.com/2014/09/mining-bitcoin-with-pencil-and-paper.html) by Ken Shirriff in which he mines Bitcoin by hand.
